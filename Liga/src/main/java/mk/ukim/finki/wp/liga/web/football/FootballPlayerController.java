...
public class FootballPlayerController {
    ...
    public String showAllPlayers(Model model) {
        ...
        model.addAttribute("bodyContent", "football/football_players");
        return "football/master_template";
    }

    @GetMapping("/add-form")
    public String addPlayerForm(@RequestParam(value = "returnUrl", required = false, defaultValue = "/football/players") String returnUrl, Model model) {
        List<FootballTeam> teams = this.footballTeamService.listAllTeams();
        model.addAttribute("teams", teams);
        model.addAttribute("returnUrl", returnUrl);
        model.addAttribute("bodyContent", "football/add_football_player");
        return "football/master_template";
    }
    ...
    public String addPlayer(@RequestParam(value = "playerImage", required = false) MultipartFile playerImage,
        ...
        FootballTeam team1 = team != null ? this.footballTeamService.findById(team) : null;
        Date birthDate = Date.from(playerBirthDate.atStartOfDay(ZoneId.systemDefault()).toInstant());
        this.footballPlayerService.create(imageBytes, playerName, playerSurname, birthDate, playerIndex, playerCity, playerPosition, team1);
    ...
    public String editFootballPlayer(@PathVariable Long id, Model model) {
        ...
        if (player != null) {
            ...
            model.addAttribute("bodyContent", "football/edit_football_player");
            return "football/master_template";
        }
        return "redirect:/football/players?error=PlayerNotFound";
    }
    ...
    public String editFootballPlayer(@PathVariable("id") Long id,
        ...
        String imageUrl = "/football/players/image/" + id;
    ...
    public String getPlayerDetails(@PathVariable Long id, HttpServletRequest request, Model model) {
        ...
        String imageUrl = "/football/players/image/" + id;
        model.addAttribute("playerImageUrl", imageUrl);
        model.addAttribute("bodyContent", "football/football_player_details");
        return "football/master_template";
    }
